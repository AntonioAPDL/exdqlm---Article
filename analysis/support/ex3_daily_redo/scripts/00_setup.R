suppressWarnings(suppressMessages({
  if (!requireNamespace("yaml", quietly = TRUE)) {
    stop("Package 'yaml' is required for analysis/support/ex3_daily_redo.")
  }
  if (!requireNamespace("pkgload", quietly = TRUE)) {
    stop("Package 'pkgload' is required for analysis/support/ex3_daily_redo.")
  }
  if (!requireNamespace("ggplot2", quietly = TRUE)) {
    stop("Package 'ggplot2' is required for analysis/support/ex3_daily_redo.")
  }
  if (!requireNamespace("gridExtra", quietly = TRUE)) {
    stop("Package 'gridExtra' is required for analysis/support/ex3_daily_redo.")
  }
  if (!requireNamespace("scales", quietly = TRUE)) {
    stop("Package 'scales' is required for analysis/support/ex3_daily_redo.")
  }
}))

if (!exists("redo_root", inherits = FALSE)) {
  stop("redo_root not defined. Run this workflow through analysis/support/ex3_daily_redo/run_all.R")
}
