...
Rscript analysis/support/ex3_daily_redo/run_all.R
```
...
- `config_full_history_q7_refresh_100.yml`
  - prepared full-history refresh run with the revised discount-factor grid
  - uses the full `1987-05-29` to `2022-12-25` window and all seven quantiles
  - keeps the `30`-day fixed-horizon holdout starting `2022-12-26`
  - uses `n.samp = 1000`, `max_iter = 100`, and `workers = 7`
  - outputs written to `outputs/full_history_q7_discount_refresh_100/`
- `config_full_history_q7_refresh_lag3_launchcheck.yml`
  - lightweight end-to-end validation run for the lagged-covariate refresh
  - uses the 2020-to-2022 window, two quantiles, and the same lagged feature recipe
  - keeps the `30`-day fixed-horizon holdout starting `2022-12-26`
  - uses `n.samp = 40`, `max_iter = 4`, and `workers = 2`
  - outputs written to `outputs/full_history_q7_discount_refresh_lag3_launchcheck/`
- `config_full_history_q7_refresh_lag3_150.yml`
  - prepared full-history refresh run with the revised discount-factor grid and lagged covariates
  - uses the full `1987-05-29` to `2022-12-25` window and all seven quantiles
  - keeps the `30`-day fixed-horizon holdout starting `2022-12-26`
  - uses `n.samp = 1000`, `max_iter = 150`, and `workers = 7`
  - outputs written to `outputs/full_history_q7_discount_refresh_lag3_150/`
...
- a configurable covariate recipe. The base recipe is the 5 standardized covariates:
  - `ppt`
  - `soil`
  - `ppt_soil`
  - `ppt2`
...
The lagged refresh configs extend that base recipe with lags `1`, `2`, and `3`
for every base covariate. That produces `20` total covariates:

- 5 contemporaneous terms
- 15 lagged terms
...
```bash
Rscript analysis/support/ex3_daily_redo/run_all.R --targets prep,fit
Rscript analysis/support/ex3_daily_redo/run_all.R --targets forecast,figures,manifest
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_smoke.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_mid.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_smoke_stable.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_smoke_stable_400.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_mid_stable.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_long_nightly_1000.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_full_history_q7_launchcheck.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_full_history_q7_nightly_1000.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_full_history_q7_refresh_100.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_full_history_q7_refresh_lag3_launchcheck.yml
Rscript analysis/support/ex3_daily_redo/run_all.R --config analysis/support/ex3_daily_redo/config_full_history_q7_refresh_lag3_150.yml
./analysis/support/ex3_daily_redo/run_long_nightly.sh
./analysis/support/ex3_daily_redo/run_full_history_q7_nightly.sh
./analysis/support/ex3_daily_redo/run_full_history_q7_refresh_100.sh
./analysis/support/ex3_daily_redo/run_full_history_q7_refresh_lag3_150.sh
Rscript analysis/support/ex3_daily_redo/run_all.R --config /path/to/config.yml
EX3_DAILY_PKG_PATH=/path/to/exdqlm Rscript analysis/support/ex3_daily_redo/run_all.R
EX3_DAILY_DATA_PATH=/path/to/big_trees_daily_usgs_ppt_soil.csv \
  Rscript analysis/support/ex3_daily_redo/run_all.R
...
```bash
Rscript analysis/support/ex3_daily_redo/run_all.R \
  --config analysis/support/ex3_daily_redo/config_full_history_q7_nightly_1000.yml \
...
```bash
bash analysis/support/ex3_daily_redo/run_full_history_q7_refresh_100.sh
```

That helper is intentionally manual. It will:

- remove the old heavyweight `.rds` cache files under `outputs/full_history_q7_nightly_1000/cache/`
- clear any stale runtime files under `outputs/full_history_q7_discount_refresh_100/`
- launch the refreshed seven-quantile full-history run under `setsid`
- write a PID file, console log, and progress log for monitoring

When you are ready to launch the lagged-covariate full-history rerun, use:

```bash
bash analysis/support/ex3_daily_redo/run_full_history_q7_refresh_lag3_150.sh
```

That helper is also intentionally manual. It will:

- remove all cached `.rds`, `.rda`, and `.RData` files under `analysis/support/ex3_daily_redo/outputs/`
- clear any stale runtime files under `outputs/full_history_q7_discount_refresh_lag3_150/`
- launch the lagged seven-quantile full-history run under `setsid`
- write a PID file, console log, and progress log for monitoring
