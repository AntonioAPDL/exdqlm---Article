...
PKG_ROOT="${EX3_MONTHLY_PKG_PATH:-$(cd "$ARTICLE_ROOT/.." && pwd)/exdqlm__wt__1.0.0-jss}"
...
if [[ "$pkg_branch" != "feature/1.0.0-jss" ]]; then
  echo "Expected package branch feature/1.0.0-jss, found $pkg_branch at $PKG_ROOT" >&2
  exit 1
fi
