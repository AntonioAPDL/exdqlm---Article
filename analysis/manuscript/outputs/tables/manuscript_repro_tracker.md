...
- [reproduced] `tab_ex2_df_scan_ldvb` -> `analysis/manuscript/outputs/tables/ex2_df_scan_kl_ldvb.csv` (new: Example 2 discount-factor KL selection (LDVB)). Best pair in this run: (0.90, 0.95)
...
- [reproduced] `tab_ex3_lambda_scan_ldvb` -> `analysis/manuscript/outputs/tables/ex3_lambda_scan_kl_ldvb.csv` (new: Example 3 lambda selection output (LDVB)). Best LDVB lambda in this run=0.900
...
- ex2_ldvb: Sunspots LDVB KL search best seasonal discount factor=0.95 for this run profile.
- ex3_ldvb: Best lambda by KL for LDVB in this run profile: 0.900.
