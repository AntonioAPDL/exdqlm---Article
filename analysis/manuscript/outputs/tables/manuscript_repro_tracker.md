...
- [reproduced] `fig_ex2checks_ldvb` -> `analysis/manuscript/outputs/figures/ex2checks_ldvb.png` (new: fig ex2checks LDVB counterpart). LDVB counterpart of ex2checks using diagnostics().
...
- [reproduced] `tab_ex2_diagnostics_ldvb` -> `analysis/manuscript/outputs/tables/ex2_diagnostics_summary_ldvb.csv` (new: Example 2 diagnostic narrative (LDVB)). LDVB counterpart computed with diagnostics().
...
- coverage: Targeted run; requested targets: ex3.
- coverage: Targeted run; requested targets: ex2.
