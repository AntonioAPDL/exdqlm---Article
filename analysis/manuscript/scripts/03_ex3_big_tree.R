...
save_png_plot("ex3quantcomps.png", {
  ...
  stats::plot.ts(y_log, col = "grey70", ylim = c(1, 8), xlim = c(1970, 1990), ylab = "quantile 95% CrIs")
  ...
  graphics::plot(NA, ylim = c(-1.5, 1.5), xlim = c(1970, 1990), ylab = "seasonal components")
  exdqlm::compPlot(M1, index = c(2, 3), add = TRUE, col = "purple")
  exdqlm::compPlot(M2, index = c(2, 3), add = TRUE, col = "forestgreen")

  graphics::plot(NA, ylim = c(-0.5, 1.5), xlim = c(1970, 1990), ylab = "Nino 3.4 components")
...
register_artifact(
  ...
  status = "reproduced",
  notes = "Three-panel quantile/components figure."
)
...
save_png_plot("ex3forecast.png", {
  stats::plot.ts(y_log, col = "grey70", ylim = c(1, 8), xlim = c(2017, 2021.4))
  exdqlm::exdqlmForecast(start.t = length(y_log) - 18, k = 18, m1 = M1, plot = TRUE, add = TRUE)
  exdqlm::exdqlmForecast(start.t = length(y_log) - 18, k = 18, m1 = M2, add = TRUE, cols = c("forestgreen", "green"))
