...
if (!need_ex1) {
  ...
  if (need_ex1kernel) {
    ...
    ex1_kernel <- load_or_fit_cache("ex1_kernel_compare_v1", {
    ...
    kernel_compare_note <- sprintf(
      paste(
        ...
        "Sigma is fixed at 0.4, so the comparison targets gamma mixing only.",
    ...
    save_table_csv(
      ...
      status = "reproduced",
      notes = "Gamma-only pooled posterior, runtime, Rhat, and ESS summaries for slice and laplace_rw with sigma fixed."
    )
    ...
    save_table_csv(
      ...
      status = "reproduced",
      notes = "Per-chain gamma posterior summaries, runtimes, and acceptance diagnostics."
    )
    ...
    save_png_plot("ex1_kernel_compare.png", {
      ...
      plot_gamma_kernel <- function(diag_obj, main, subtitle = NULL) {
        iter <- seq_len(length(diag_obj$fits[[1L]]$gamma))
        gamma_mat <- do.call(cbind, lapply(diag_obj$fits, `[[`, "gamma"))
        graphics::matplot(
          iter,
          gamma_mat,
          type = "l",
          lty = 1,
          lwd = 1,
          col = chain_cols[seq_len(ncol(gamma_mat))],
          xlab = "kept iteration",
        ...
        graphics::legend(
          "topright",
          legend = sprintf("chain %d", seq_len(ncol(gamma_mat))),
          col = chain_cols[seq_len(ncol(gamma_mat))],
          lty = 1,
          bty = "n", cex = 0.8
        )
      ...
      plot_gamma_density <- function(diag_obj, main, subtitle = NULL) {
        dens_list <- lapply(diag_obj$fits, function(fit_i) stats::density(fit_i$gamma, n = 512))
        xlim <- range(unlist(lapply(dens_list, `[[`, "x"), use.names = FALSE), finite = TRUE)
        ylim <- range(unlist(lapply(dens_list, `[[`, "y"), use.names = FALSE), finite = TRUE)
        graphics::plot(
          dens_list[[1L]],
          type = "n",
          xlim = xlim,
        ...
        for (i in seq_along(dens_list)) {
          graphics::lines(dens_list[[i]], col = chain_cols[i], lwd = 1.2)
        }
        graphics::abline(v = diag_obj$summary_row$gamma_mean, col = "black", lty = 2)
        graphics::legend(
          "topright",
          legend = c(sprintf("chain %d", seq_along(dens_list)), "pooled mean"),
          col = c(chain_cols[seq_along(dens_list)], "black"),
          lty = c(rep(1, length(dens_list)), 2),
          bty = "n", cex = 0.8
        )
      }
      ...
      plot_gamma_density(
        ex1_kernel$slice,
        main = "slice: gamma densities",
        subtitle = "sigma fixed at 0.4"
      )
      plot_gamma_density(
        ex1_kernel$laplace_rw,
        main = "laplace_rw: gamma densities",
        subtitle = "sigma fixed at 0.4"
      )
    })
    register_artifact(
      ...
      status = "reproduced",
      notes = "Four-chain Lake Huron median comparison with gamma trace and density overlays under fixed sigma."
    )
