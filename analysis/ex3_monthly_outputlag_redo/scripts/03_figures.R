...
labels_compatible <- function(values, expected_levels) {
  if (!length(values)) return(FALSE)
  all(unique(as.character(values)) %in% as.character(expected_levels))
}
...
build_direct_state_data <- function() {
  ...
  if (isTRUE(figure_cache_valid) &&
    ...
    cached_df$period_label <- factor(cached_df$period_label, levels = periods$period_label)
    cached_df$tau_label <- factor(cached_df$tau_label, levels = tau_levels)
    cached_df$state_label <- factor(cached_df$state_label, levels = expected_state_levels)
    return(cached_df)
  }
...
fit_period_plot <- ggplot2::ggplot() +
  ...
  ggplot2::scale_fill_manual(values = quant_cols) +
  ggplot2::labs(
    title = "Monthly output-lag contrast fits across the selected windows",
    subtitle = sprintf(
...
render_state_figure(
  ...
  title = "Transfer-model state paths during the 1997-1999 comparison window"
)
render_state_figure(
  df = transfer_state_data,
...
render_state_figure(
  ...
  title = "Direct-model regression state paths during the 1997-1999 comparison window"
)
render_state_figure(
  df = direct_state_data,
  period_label = periods$period_label[periods$period == "drought"],
...
make_convergence_plot(
  df = convergence_df,
  value_col = "elbo",
  title = "Monthly output-lag contrast: ELBO traces",
  filename = "ex3_monthly_convergence_elbo.png"
)
make_convergence_plot(
  df = convergence_df,
  value_col = "sigma",
  title = "Monthly output-lag contrast: sigma traces",
  filename = "ex3_monthly_convergence_sigma.png"
)
make_convergence_plot(
  df = convergence_df,
  value_col = "gamma",
  title = "Monthly output-lag contrast: gamma traces",
