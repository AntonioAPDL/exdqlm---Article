...
manifest_lines <- c(
  ...
  sprintf("- LDVB settings: tol=%s, n.samp=%s, gam.init=%s, sig.init=%s",
...
status_lines <- apply(fit_summary, 1, function(row) {
  sprintf("- p0=%s | %s | status=%s | runtime=%s | median.kt=%s",
          row[["p0"]], row[["model"]], row[["status"]], row[["runtime"]], row[["median_kt"]])
})
