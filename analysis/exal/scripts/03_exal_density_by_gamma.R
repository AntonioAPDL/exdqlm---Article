...
for (i in seq_along(gamma_vals)) {
  g <- gamma_vals[[i]]
  d <- exdqlm::dexal(x, p0 = p0, mu = mu, sigma = sigma, gamma = g)
  rows[[i]] <- data.frame(
    x = x,
    density = d,
    gamma = g,
...
plot_obj <- ggplot(df, aes(x = x, y = density, color = gamma_label, linetype = gamma_label)) +
  geom_line(linewidth = 1) +
  scale_color_manual(values = palette_vals[1:3]) +
  scale_linetype_manual(values = c("solid", "dashed", "dotdash")) +
  labs(
    title = "exAL Density Under Gamma Variation",
...
save_plot_file(
  plot_obj,
  filename = "exal_fig_03_exal_density_gamma_sweep.png",
  description = "exAL density curves under negative, zero, and positive gamma",
  main_text_candidate = TRUE
)

save_table_file(
  df,
  filename = "exal_data_03_exal_density_gamma_sweep.csv",
  description = "Underlying data for exAL density gamma sweep"
)
