...
Readers interested in MCMC implementation should focus on Sections
\ref{sec:dyn_dqlm}, \ref{sec:dyn_exdqlm}, \ref{sec:transfer_function}, and
\ref{sec:static_ridge}. Readers interested in LDVB should focus on Sections
\ref{sec:dyn_dqlm}, \ref{sec:dyn_exdqlm}, \ref{sec:static_ridge},
\ref{sec:static_rhs}, and \ref{sec:numerical_blocks}. Readers checking package
outputs should use Table~\ref{tab:supp_article_crosswalk} and
Table~\ref{tab:supp_algorithm_map}.
...
Numerical blocks & Section 2.2 and package implementation & S9 & Slice sampler, Laplace-Delta, ELBO blocks \\
\bottomrule
\end{tabularx}
...
Therefore Algorithms S3 and S4 apply after replacing
...
\toprule
Mathematical object & Package input \\
\midrule
$x_t$ & \code{X} \\
$\lambda$ & \code{lam} \\
...
median decay time & \code{median.kt} \\
\bottomrule
\end{tabularx}
\caption{Transfer-function quantities and their corresponding package inputs.}
...
The dynamic diagnostics use the fitted one-step-ahead sequence produced by the
filtering recursion. In particular, the PIT value reported at time $t$ is
\begin{equation}
\widehat u_t =
\Pr\{Y_t\le y_t\mid y_{1:t-1},
\widehat v_{1:T},\widehat s_{1:T},\widehat\sigma,\widehat\gamma\},
\label{eq:pit_plugin}
\end{equation}
where hats denote the fitted plug-in summaries used by the diagnostic routine.
The QQ plot, ACF plot, KL divergence, and standardized forecast-error plot are
computed from this diagnostic sequence rather than from posterior uncertainty
bands for residuals.
...
Dynamic diagnostic summaries are returned by \code{exdqlmDiagnostics()}.
The static diagnostic routine returns check-loss summaries and
reference-quantile errors when a reference quantile is supplied.
...
distribution. The monotonicity correction uses isotonic adjustment or monotone
rearrangement \citep{barlow1972,chernozhukov2010}.
