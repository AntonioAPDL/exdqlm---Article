...
numerical routines used by the package. It is primarily for readers who want
to verify the posterior targets, update equations, and diagnostic quantities
...
model sections derive the fitting targets and updates,
Section~\ref{sec:numerical_blocks} collects reusable numerical identities, and
Table~\ref{tab:supp_algorithm_map} links the exported functions to the
corresponding computational blocks.
...
Forecasting & Equation (15) & S8.1 \\
Diagnostics & Equations (11)--(14) & S8.2 \\
...
corresponding derivation sections.}
...
Users specify discounting through \code{df} and \code{dim.df}; the routines
construct the corresponding $\mat{W}_t$ sequence internally, and the notation
above writes the induced covariance generically as $\mat{W}_t$.
...
Gaussian/ridge prior and the conjugate RHS-NS prior, both for MCMC and LDVB.
...
\code{beta_prior = "rhs_ns"}. The prior enters the static Gaussian update for
...
\texttt{exdqlmDiagnostics()} & S8.2 & MAP one-step-ahead diagnostic sequence, CRPS, and pplc. \\
...
\end{tabularx}
\endgroup
\caption{Mapping between exported \pkg{exdqlm} functions and the derivation or
computational details documented in this supplement.}
