...
\begin{abstract}
This document records the model hierarchies, augmented posterior targets, MCMC
updates, variational approximations, and ELBO components underlying the main
dynamic models and the static ridge/RHS-NS paths in the current \pkg{exdqlm}
implementation. It is written as a working supplement draft:
sections that have already been checked against the existing theory notes are
presented in mathematical form, and the final verification tasks are listed at
the end. Vectors are written in bold lowercase, matrices in bold uppercase, and
dimensions are given when each object is introduced.
\end{abstract}
...
A_\gamma &= A\{p(\gamma)\}, \qquad
B_\gamma = B\{p(\gamma)\}, \qquad
C_\gamma = \{\Ind(\gamma>0)-p(\gamma)\}^{-1}.
\label{eq:exal_ABC}
\end{align}
...
\]
and a proper truncated Student-$t$ prior for $\gamma$ on $(L,U)$.
...
% Source check:
% AL/DQLM: /home/jaguir26/local/src/DQLM-and-BQR---Theory/main.tex lines 39-64.
% exAL: /home/jaguir26/local/src/univ-exDQLM---Ensemble/main.tex lines 108-143.
...
Given $(\sigma,\vect{v})$, sample
$\vect{\theta}_{0:T}$ exactly by forward-filtering backward-sampling (FFBS)
applied to \eqref{eq:dyn_dqlm_pseudo}.
...
% Source check:
% /home/jaguir26/local/src/DQLM-and-BQR---Theory/main.tex lines 746-1191.
% Current package reduced path:
% /home/jaguir26/local/src/exdqlm__wt__rhs_ns_reconcile/R/exdqlmLDVB.R lines 227-245.
...
&\quad\times
\prod_{t=1}^Tp(v_t\mid\sigma)\prod_{t=1}^Tp(s_t)\,p(\sigma)p(\gamma).
...
The current package default samples $\sigma$ from its exact conditional
...
Jacobian $u+\log\{\dd\gamma/\dd\xi\}$. The package stores the transformed mode,
the approximate covariance matrix, Delta-method moments, and ELBO components.
...
% Source check:
% /home/jaguir26/local/src/univ-exDQLM---Ensemble/main.tex lines 108-790.
% Package slice default:
% /home/jaguir26/local/src/exdqlm__wt__rhs_ns_reconcile/R/exdqlmMCMC.R lines 1200-1270.
...
\subsection{LDVB Updates and ELBO}
...
\begin{equation}
\elbo(q)=
\E_q\{\log p(\vect{\beta})+\log p(\sigma)+\log p(\gamma)\}
...
% Source check:
% /home/jaguir26/local/src/exAL---Regression/main.tex lines 53-1790.
% /home/jaguir26/local/src/DQLM-and-BQR---Theory/main.tex lines 68-530.
...
Gibbs-friendly regularized horseshoe variant used as the sparse-prior target
for this supplement. It enters the static Gaussian update for $\vect{\beta}$
through a prior precision matrix. Let
...
developed here because its log-scale block is nonconjugate and is not the
intended sparse-prior derivation for this supplement.

\subsection{RHS-NS Prior: Package-exact Hierarchy}

The \code{rhs\_ns} option is the Gibbs-friendly regularized horseshoe variant
used in the current implementation. For $j\in\mathcal{A}$,
\begin{align}
...
% Source check:
% RHS-NS package code:
% /home/jaguir26/local/src/exdqlm__wt__rhs_ns_reconcile/R/static_beta_prior.R lines 989-1470.
% Conceptual RHS notes:
% /home/jaguir26/local/src/RHS---Implementations/main.tex lines 202-865.
% Package-exact RHS-NS TeX source:
% /home/jaguir26/local/src/Q-DESN---Theory-for-implementation/main.tex lines 91-183, 240-284, 369-463, 501-540.
...
utility is also available to internal one-dimensional nonconjugate blocks, but
those legacy shrinkage targets are outside the scope of this supplement.
...
% Source check:
% Slice code:
% /home/jaguir26/local/src/exdqlm__wt__rhs_ns_reconcile/R/utils.R lines 239-330.
% Laplace-Delta sources:
% /home/jaguir26/local/src/univ-exDQLM---Ensemble/main.tex lines 549-658.
% /home/jaguir26/local/src/exAL---Regression/main.tex lines 1019-1412.
...
\toprule
Function & Model & Main inference target & Notes \\
\midrule
\code{exdqlmMCMC()} & Dynamic DQLM/exDQLM & Posterior simulation & FFBS plus Gibbs/slice blocks \\
\code{exdqlmLDVB()} & Dynamic DQLM/exDQLM & Mean-field VB & CAVI plus Laplace-Delta \\
\code{exalStaticMCMC()} & Static AL/exAL & Posterior simulation & Ridge/RHS-NS priors \\
\code{exalStaticLDVB()} & Static AL/exAL & Mean-field VB & Ridge/RHS-NS priors \\
\bottomrule
\end{tabularx}
\end{center}

\section{Second-pass Verification Checklist}

Before this supplement is submitted, perform the following checks:
\begin{enumerate}[label=(\arabic*)]
\item Compare every displayed DQLM expression against
\code{DQLM-and-BQR---Theory/main.tex}.
\item Compare every dynamic exDQLM expression against
\code{univ-exDQLM---Ensemble/main.tex} and the current
\code{exdqlmMCMC()} / \code{exdqlmLDVB()} implementation.
\item Compare every static exAL ridge expression against
\code{exAL---Regression/main.tex}.
\item Verify the term-by-term RHS-NS ELBO against
\code{static\_beta\_prior.R}.
\end{enumerate}
