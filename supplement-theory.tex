...
\paragraph{Algorithm S1 (dynamic DQLM MCMC).}\hfill\par
...
\paragraph{Algorithm S2 (dynamic DQLM LDVB).}\hfill\par
\par\noindent\emph{Input:} observations $\vect{y}$, model matrices
$(\mat{F}_t,\mat{G}_t,\mat{W}_t)$, initial state prior, AL prior
...
\paragraph{Algorithm S3 (dynamic exDQLM MCMC).}\hfill\par
...
\paragraph{Algorithm S4 (dynamic exDQLM LDVB).}\hfill\par
\par\noindent\emph{Input:} observations $\vect{y}$, model matrices
$(\mat{F}_t,\mat{G}_t,\mat{W}_t)$, initial state prior, exAL prior
...
\paragraph{Algorithm S5 (transfer-function state augmentation and fitting).}\hfill\par
...
MCMC uses Algorithm S3 and LDVB uses Algorithm S4.
...
\paragraph{Algorithm S6 (static AL/exAL MCMC).}\hfill\par
...
\paragraph{Algorithm S7 (static AL/exAL LDVB).}\hfill\par
\par\noindent\emph{Input:} response vector $\vect{y}$, design matrix
$\mat{X}$, target quantile $p_0$, coefficient prior specification, AL/exAL
...
\paragraph{Algorithm S8 (posterior-predictive synthesis).}\hfill\par
...
\texttt{exdqlmTransferMCMC()} & S5 & Transfer-function state augmentation; Algorithm S5 with Algorithm S3. \\
\texttt{exdqlmTransferLDVB()} & S5 & Transfer-function state augmentation; Algorithm S5 with Algorithm S4. \\
