...
\subsection{Reusable ELBO building blocks}
\label{sec:elbo_building_blocks}

The package records an ELBO sequence for VB convergence monitoring. All
ELBOs in this supplement use
\begin{equation}
\elbo(q)=\E_q\{\log p(\text{complete data})\}+H(q),
\qquad
H(q)=-\E_q\{\log q\}.
\end{equation}
The following entropy and moment formulas are reused across the dynamic and
static algorithms.
...
\end{equation}
If $x\sim\IG(a,b)$ under the rate parameterization stated above, then
\begin{equation}
...
If $x\sim\GIG(\lambda,\chi,\psi_g)$ and
$z=(\chi\psi_g)^{1/2}$, let
$m_x=\E(x)$, $m_{1/x}=\E(x^{-1})$, and
$m_{\log x}=\E(\log x)$. Then
\begin{align}
H\{\GIG(\lambda,\chi,\psi_g)\}
&=
\log\{2K_\lambda(z)\}
-\frac{\lambda}{2}\log\left(\frac{\psi_g}{\chi}\right)
-(\lambda-1)m_{\log x} \nonumber\\
&\quad+
\frac{1}{2}\{\chi m_{1/x}+\psi_g m_x\}.
...
$\lambda$ plus $\frac{1}{2}\log(\chi/\psi_g)$, and is evaluated numerically in
the package.

If $x\sim\TNp(m,V)$, let $s=V^{1/2}$, $u=m/s$, and
$\Lambda(u)=\phi(u)/\Phi(u)$. The entropy is
\begin{equation}
H\{\TNp(m,V)\}
=
\frac{1}{2}\log(2\pi V)+\log\Phi(u)
+\frac{1}{2}\{1-u\Lambda(u)\}.
...
\end{equation}
If $\vect{z}=(z_\gamma,u)^\T$ has Laplace approximation
...
\begin{equation}
\log|J_T(\vect{z})|
=u+\log(U-L)+\log r+\log(1-r),
\qquad r=\expit(z_\gamma).
\end{equation}
The final expectation in \eqref{eq:entropy_ld}, and the other smooth
expectations of functions of $(\sigma,\gamma)$, are evaluated by the same
second-order Delta approximation used in the LDVB updates.
...
\sum_{t=1}^T\E_q\{\log p(y_t\mid\vect{\theta}_t,\sigma,v_t)\}
&=
-\frac{T}{2}\log(2\pi)-\frac{T}{2}\log B_0
-\frac{T}{2}\bar\ell_\sigma
-\frac{1}{2}\sum_{t=1}^T\bar\ell_{v,t}\nonumber\\
&\quad
-\frac{\kappa}{2B_0}\sum_{t=1}^T
\left\{\ell_t\overline{r_t^2}-2A_0\bar r_t+A_0^2\nu_t\right\}.
...
\begin{equation}
t_i=y_i-\vect{x}_i^\T\vect{m}_\beta,\qquad
q_i=\vect{x}_i^\T\mat{\Sigma}_\beta\vect{x}_i.
\end{equation}
Then
\begin{align}
\elbo(q)
...
The formulas above describe the conditional targets and variational coordinate
targets used by the package. The current implementation also exposes numerical
warmup and scheduling controls, for example temporary holds of selected latent,
state, or $(\sigma,\gamma)$ blocks in difficult early iterations. These controls
change only when a target is updated, not the model or the target distributions
displayed in this supplement.
...
\midrule
\code{exdqlmMCMC()} & Dynamic DQLM/exDQLM posterior simulation: FFBS for states, Gibbs for conjugate latent/scale blocks, and a default bounded-slice update for $\gamma$ when exAL is used, with random-walk alternatives exposed. \\
...
\code{exalStaticMCMC()} & Static AL/exAL posterior simulation: Gaussian coefficient update, Gibbs latent/scale updates, RHS-NS inverse-gamma scale updates when selected, and a default bounded-slice update for $\gamma$ in exAL, with random-walk alternatives exposed. \\
