...
$\overline{s_t^2}=\E(s_t^2)$. The state factor is Gaussian and is computed by Kalman
...
Algorithms~\ref{alg:dyn-exdqlm-mcmc} and~\ref{alg:dyn-exdqlm-ldvb} apply after replacing
...
The dynamic diagnostics use the plug-in MAP standardized one-step-ahead
...
\label{eq:pit_plugin}
\end{equation}
The QQ plot, ACF plot, and standardized forecast-error plot are computed from
$\{\widehat e_t\}$ or $\{\widehat u_t\}$ rather than from posterior uncertainty
bands for residuals. The reported $\KL$ summaries are nearest-neighbor estimates
...
posterior predictive loss criterion returned as \code{pplc} is
\begin{equation}
\widehat{\PPLC}
=
\sum_t
...
\midrule
\code{exdqlmMCMC()} & S3--S4 & Dynamic DQLM/exDQLM MCMC; Algorithms~\ref{alg:dyn-dqlm-mcmc} and~\ref{alg:dyn-exdqlm-mcmc}. \\
\code{exdqlmLDVB()} & S3--S4 & Dynamic DQLM/exDQLM LDVB; Algorithms~\ref{alg:dyn-dqlm-vb} and~\ref{alg:dyn-exdqlm-ldvb}. \\
\code{exdqlmTransferMCMC()} & S5 & Transfer-function state augmentation; Algorithm~\ref{alg:transfer-fitting} with Algorithm~\ref{alg:dyn-exdqlm-mcmc}. \\
\code{exdqlmTransferLDVB()} & S5 & Transfer-function state augmentation; Algorithm~\ref{alg:transfer-fitting} with Algorithm~\ref{alg:dyn-exdqlm-ldvb}. \\
\code{exalStaticMCMC()} & S6--S7 & Static AL/exAL MCMC; Algorithm~\ref{alg:static-mcmc}. \\
\code{exalStaticLDVB()} & S6--S7 & Static AL/exAL LDVB; Algorithm~\ref{alg:static-ldvb}. \\
\code{exdqlmForecast()} & S8.1 & Forecast recursion and predictive draws. \\
\code{exdqlmDiagnostics()} & S8.2 & MAP one-step-ahead diagnostic sequence, $\CRPS$, and \code{pplc}. \\
\code{exalStaticDiagnostics()} & S8.2 & Static check-loss and reference-quantile diagnostics. \\
\code{quantileSynthesis()} & S8.3 & Posterior-predictive synthesis; Algorithm~\ref{alg:predictive-synthesis}. \\
\begin{tabular}[t]{@{}l@{}}\code{dexal()}, \code{pexal()}\\ \code{qexal()}, \code{rexal()}\end{tabular} & S2 & exAL distribution evaluation and simulation. \\
\code{get\_gamma\_bounds()} & S2 & Roots $L,U$ for a fixed $p_0$. \\
\bottomrule
\end{tabularx}
\endgroup
