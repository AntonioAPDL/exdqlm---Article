...
\caption{Example 1: Lake Huron. Trace and density plots from the median run with 7000 burn-in iterations and 3000 retained draws. For readability, every 10th retained draw is used in the plotting step. The top row shows the posterior trace plots for \(\sigma\) and \(\gamma\), and the bottom row shows the corresponding posterior densities. The displayed paths are representative MCMC traces from the recorded reference run; exact trace paths may differ across reruns while retaining comparable posterior summaries.}
\label{fig:ex1mcmc}
\end{figure}
...
\begin{tabular}{@{}llrrrr@{}}
 ...
 \midrule
 \tworowcell{DQLM} & LDVB & \textbf{22.27} & \textbf{0.133} & 8.525 & 3851.4 \\
 & MCMC & 143.57 & 0.175 & \textbf{7.608} & \textbf{2969.0} \\
 \midrule
 \tworowcell{exDQLM} & LDVB & \textbf{42.95} & \textbf{0.107} & 6.558 & 3015.2 \\
 & MCMC & 190.29 & 0.152 & \textbf{5.147} & \textbf{2216.1} \\
 \bottomrule
\end{tabular}
...
\begin{tabular}{@{}llrrrr@{}}
 ...
 \midrule
 \tworowcell{0.05} & LDVB & \textbf{31.01} & 0.108 & \textbf{0.034} & 0.673 \\
 & MCMC & 68.15 & \textbf{0.069} & 0.035 & \textbf{0.594} \\
 \midrule
 \tworowcell{0.25} & LDVB & \textbf{27.50} & 0.310 & 0.078 & 0.371 \\
 & MCMC & 67.63 & \textbf{0.271} & \textbf{0.065} & \textbf{0.329} \\
 \midrule
\tworowcell{0.50} & LDVB & \textbf{12.39} & \textbf{0.128} & 0.103 & 0.341 \\
 & MCMC & 67.06 & 0.141 & \textbf{0.083} & \textbf{0.335} \\
 \bottomrule
\end{tabular}}
...
Table \ref{tab:ex4static} includes active-signal RMSE and null-coefficient MAE summaries stored in the \code{exalStaticDiagnostic} objects returned by \code{diagnostics()}. The results reinforce the visual comparison in Figure \ref{fig:ex4static}. The MCMC posterior-simulation fit has the smaller holdout quantile RMSE at \(p_0 = 0.05\) and \(p_0 = 0.25\), while LDVB is smaller at \(p_0 = 0.50\). MCMC gives slightly smaller active-signal RMSE in all three fits, whereas null-coefficient MAE favors LDVB for \(p_0 = 0.05\) and \(p_0 = 0.25\) and is essentially tied at the median. The LDVB fit is nevertheless faster in all three cases, with runtime reductions of roughly 2.2-fold at \(p_0 = 0.05\), 2.5-fold at \(p_0 = 0.25\), and 5.4-fold at \(p_0 = 0.50\). In this benchmark, the LDVB approximation is useful as a rapid first-pass screen, while static MCMC provides a more expensive posterior-simulation comparison when interval accuracy or posterior tail behavior is central.
