...
If $P_e$ denotes the density estimate of the MAP standardized forecast-error distribution and $\phi$ denotes the standard normal density, the package reports a forward diagnostic, denoted $\KL(P_e\,|| \ \phi)$ and, as a sensitivity measure, the reversed diagnostic $\KL(\phi \,||\,P_e)$. %The forward calculation uses the semiclosed identity based on analytic normal cross-entropy and a one-dimensional entropy estimate; the flipped calculation is reported as a secondary sensitivity diagnostic.
...
As an illustration here we fix a single discount factor of $0.9$ to define the evolution covariance matrix $\mat{W}_t$.
...
Because the posterior for $\gamma$ is concentrated around zero and the DQLM provides a simpler median specification, we use the fixed-$\gamma$ model for the remaining median analysis. We re-run the model with a point-mass prior on $\gamma$ at 0 using the settings \code{gam.init = 0} and \code{fix.gamma = TRUE}, which is equivalent to the setting \code{dqlm.ind = TRUE}. This reduces the model to the DQLM mentioned in Section \ref{sec:special}.
...
In the recorded manuscript run, \code{quantileSynthesis()} uses the package's isotonic adjustment and monotone rearrangement options, and the stored synthesized quantile summaries have no remaining crossings over the observed period.
...
County, CA \citep{btflow}.
 The \pkg{exdqlm} version 1.1.0 used for this article includes these measurements
...
script. The list \code{vb.ctrl} sets the LDVB iteration cap used
in the replication run, so the displayed code records the same convergence
control as the executable script.

\begin{CodeChunk}
\begin{CodeInput}
R> lambda.grid = c(0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 0.99)
R> vb.ctrl = list(max_iter = 600L)
...
+                 gam.init = -0.1, n.samp = 1000, tol = 0.05, vb_control = vb.ctrl, verbose = FALSE)
...
+                n.samp = 1000, tol = 0.05, vb_control = vb.ctrl, verbose = FALSE)
