...
\raggedbottom
\graphicspath{{figures/}{analysis/manuscript/outputs/figures/}}
...
The \code{exdqlm} package is distributed under the MIT License and is available from the Comprehensive R Archive Network at \url{https://CRAN.R-project.org/package=exdqlm}. The package source submitted with this article is version 1.1.2, a reproducibility patch built on the public 1.1.x package line.
...
R> plot(M50.dqlm, add = TRUE, col = "steelblue")
R> plot(M5, add = TRUE, col = "forestgreen")
R> legend("topright", lty = 1, bty = "n", col = c("purple", "steelblue", "forestgreen"),
...
R> plot(fc50, add = TRUE, cols = c("steelblue", "light blue"))
R> plot(fc05, add = TRUE, cols = c("forestgreen", "darkseagreen"))
\end{CodeInput}
\end{CodeChunk}
...
\begin{CodeChunk}
    \begin{CodeInput}
R> plot.ts(sunspot.year, col = "dark grey", ylab = "sunspot count", xlab = "year")
R> title("Sunspot time series")
    \end{CodeInput}
\end{CodeChunk}
...
R> plot(M1, add = TRUE, col = "red2")
R> plot(M2, add = TRUE, col = "steelblue")
R> legend("topleft", lty = 1, bty = "n", col = c("red2", "steelblue"), legend= c("DQLM", "exDQLM"))
...
R> title("posterior draws of gamma (p0 = 0.85)")
\end{CodeInput}
\end{CodeChunk}
...
R> plot(diagM1M2, cols = c("red2", "steelblue"))
\end{CodeInput}
\end{CodeChunk}
...
\begin{CodeOutput}
   X1   X2   CRPS    KL
1 0.9 0.85  6.567 0.107
2 0.9 0.90  8.210 0.146
3 0.9 0.95 10.854 0.173
4 0.9 1.00 14.732 0.247
\end{CodeOutput}
\begin{CodeInput}
...
County, CA \citep{btflow}. The \pkg{exdqlm} version 1.1.2 used for this
...
These data can be seen in Figure \ref{fig:ex3data} and plotted as follows.

\begin{CodeChunk}
    \begin{CodeInput}
R> par(mfrow = c(2, 1), mar = c(3.0, 4.2, 1.0, 0.8), oma = c(1.6, 0, 0, 0))
R> plot(y.fit, col = "grey35", ylab = "log flow", xlab = "", lwd = 1.1)
R> grid(col = "grey88")
R> abline(v = xy.coords(y.fit)$x[414], col = "orange", lty = 5, lwd = 1.2)
R> 
R> plot(NA, xlab = "", ylab = "standardized index", xlim = range(time(y.fit)), ylim = c(-4.3,4) )
R> lines(as.numeric(time(y.fit)), scale(X.raw[,1]), col ="steelblue", lwd = 1.6, lty = 1)
R> lines(as.numeric(time(y.fit)), scale(X.raw[,2]), col = "red2", lwd = 1.6, lty = 2)
R> grid(col = "grey88")
R> legend(
...
+ )
R> abline(v = xy.coords(y.fit)$x[414], col = "orange", lty = 5, lwd = 1.2)
R> mtext("time", side = 1, outer = TRUE, line = 0.4)
    \end{CodeInput}
\end{CodeChunk}
...
R> plot(MTF, add = TRUE, col = "forestgreen")
...
R> plot(MTF, type = "component", index = 2:7, add = TRUE, col = "forestgreen")
...
\begin{CodeChunk}
\begin{CodeInput}
R> plot(NA, ylim = c(-2.5, 3), xlim = c(2016, 2020), ylab = "covariate contribution")
R> grid(col = "grey90")
R> plot(MREG, type = "component", index = 8:9, add = TRUE, col = "steelblue")
R> plot(MTF, type = "component", index = 8, add = TRUE, col = "forestgreen")
R> abline(h = 0, col = "orange", lty = 3, lwd = 1.4)
...
R> plot(MTF, type = "state", index = 8, col = "forestgreen", add = FALSE)
R> grid(col = "grey90")
R> abline(h = 0, col = "orange", lty = 3, lwd = 1.4)
...
R> plot(y.train, type = "n", ylim = c(-0.015, 0.020), xlab = "time", ylab = "component CrIs")
...
\begin{CodeInput}
R> par(mfrow = c(1, 1))
R> plot(y.fit, col = "grey70", xlim = c(2020, 2023), ylim = c(0.5,8), xaxt = "n",
...
R> plot(fc.MTF, add = TRUE, cols = c("forestgreen", "darkseagreen"))
...
\begin{tabular}{@{}llrrrr@{}}
 ...
 \midrule
 \tworowcell{0.05} & LDVB & \textbf{31.22} & 0.108 & \textbf{0.034} & 0.673 \\
 & MCMC & 75.69 & \textbf{0.069} & 0.035 & \textbf{0.594} \\
 \midrule
 \tworowcell{0.25} & LDVB & \textbf{31.62} & 0.310 & 0.078 & 0.371 \\
 & MCMC & 74.72 & \textbf{0.271} & \textbf{0.065} & \textbf{0.329} \\
 \midrule
\tworowcell{0.50} & LDVB & \textbf{12.51} & \textbf{0.128} & 0.103 & 0.341 \\
 & MCMC & 75.75 & 0.141 & \textbf{0.083} & \textbf{0.335} \\
 \bottomrule
\end{tabular}}
...
The results reinforce the visual comparison in Figure \ref{fig:ex4static}. The MCMC posterior-simulation fit has the smaller holdout quantile RMSE in all three fits, although the median difference is small. MCMC gives smaller active-signal RMSE at \(p_0 = 0.05\) and \(p_0 = 0.25\), while LDVB is slightly smaller at \(p_0 = 0.50\). Null-coefficient MAE is slightly smaller for LDVB at \(p_0 = 0.05\) and for MCMC at \(p_0 = 0.25\) and \(p_0 = 0.50\). The LDVB fit is nevertheless faster in all three cases, with runtime reductions of roughly 2.4-fold at \(p_0 = 0.05\), 2.4-fold at \(p_0 = 0.25\), and 6.1-fold at \(p_0 = 0.50\). In this benchmark, the LDVB approximation is useful as a rapid first-pass screen, while static MCMC provides a more expensive posterior-simulation comparison when interval accuracy or posterior tail behavior is central.
...
block. In version 1.1.2, the default implementation uses a structured
