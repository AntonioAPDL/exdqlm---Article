...
As an illustration we fix a single discount factor of $0.9$ to define the evolution covariance matrix $\mat{W}_t$.
 \textcolor{red}{Discount factors can also be selected using in-smaple diagnostic criteria, as illustrated in Section \ref{sec:ex2}.}
...
\textcolor{red}{\bf RP: This is still a bit confusing. Why do we need to do this run with 7000 \code{n.burn} and 3000 \code{n.mcmc} but then when we fix the gamma below we go with  \code{n.burn=2000} and \code{n.mcmc=3000}? Either both should have the same or if a reduced number of iterations is used for \code{M50.dqlm} below it needs to say why and explain convergence was reached with a smaller number of iterations below but for the model with gamma random we needed more iterations.}
...
\textcolor{red}{In the recorded manuscript run, \code{quantileSynthesis()} uses the package's isotonic adjustment and monotone rearrangement as default options,} and the stored synthesized quantile summaries have no remaining crossings over the observed period.
...
\textcolor{red}{The notation here is different than in the main paper. In the main paper $\mathbf{x}_t$ is a $k$-dimensional input and $\zeta_t$ is one-dimensional. Please clarify}. 
