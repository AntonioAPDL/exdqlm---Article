...
The manuscript results were generated with \proglang{R} 4.6.0 and \pkg{exdqlm} 1.1.1 on x86\_64 Linux. The replication scripts set \code{RNGversion("4.6.0")} and \code{RNGkind("Mersenne-Twister", "Inversion", "Rejection")} before applying the recorded seeds. Native thread variables for OpenMP and common BLAS implementations are set to one before launching \proglang{R}; the benchmark profile used in the examples enables the \code{"fast"} C++ MCMC backend and keeps stochastic compiled helper streams controlled by the \proglang{R} random-number generator. The submitted replication archive contains a flat \code{code.R} script intended to be run with \code{R CMD BATCH --vanilla code.R code.Rout}. That run prints fitted-object output, Tables 7--10, selected scalar summaries, and \code{sessionInfo()} to \code{code.Rout}, and writes the manuscript figures to \code{Rplots.pdf} and to the PNG files used by this article. A reduced \code{code-fast.R} script checks the same code paths with smaller computation settings; it is not used for manuscript numerical values. Runtime entries in the tables are elapsed fitting times on the recorded reference platform and are not intended to be machine-independent constants.
...
\end{CodeInput}
\begin{CodeOutput}
Dynamic quantile state-space fit
Model: exDQLM (exAL)
Inference engine: MCMC
Quantile level (p0): 0.95
Observations: 98
State dimension: 2
Discount factors (dimensions): 0.9 ( 2 )
Burn-in: 2000
Posterior draws: 3000
State draws: 2 x 98 x 3000
Posterior predictive draws: 98 x 3000
Run-time: 87.505 seconds

Class: "exdqlmMCMC" "exdqlmFit"
Use with: summary(), plot(), predict(), diagnostics()
Plot types: quantile, component, state
\end{CodeOutput}
\begin{CodeInput}
R> summary(M95)
\end{CodeInput}
...
Stored draws:
                ...
                state draws 2 x 98 x 3000
 posterior predictive draws     98 x 3000
                sigma draws      3000 x 1
                gamma draws      3000 x 1

Scalar posterior summaries:
 Parameter    Mean      SD
     sigma  0.2101 0.02172
     gamma -4.1364 0.79171
\end{CodeOutput}
\end{CodeChunk}
...
County, CA \citep{btflow}. The \pkg{exdqlm} version 1.1.1 used for this
...
\begin{tabular}{@{}lrrr@{}}
 ...
 Model & KL & CRPS & PPLC \\
 \midrule
 \code{M0} no covariates & 0.168 & 0.470 & \textbf{99.340} \\
 \code{MREG} direct regression & \textbf{0.112} & \textbf{0.348} & 151.792 \\
 \code{MTF} transfer function & 0.155 & 0.366 & 125.391 \\
 \bottomrule
\end{tabular}
...
\begin{tabular}{@{}llrrrr@{}}
 ...
 \midrule
 \tworowcell{0.05} & LDVB & \textbf{20.97} & 0.107 & \textbf{0.034} & 0.674 \\
 & MCMC & 80.42 & \textbf{0.072} & 0.040 & \textbf{0.598} \\
 \midrule
 \tworowcell{0.25} & LDVB & \textbf{10.69} & 0.174 & \textbf{0.071} & 0.426 \\
 & MCMC & 78.61 & \textbf{0.160} & 0.084 & \textbf{0.419} \\
 \midrule
\tworowcell{0.50} & LDVB & \textbf{5.97} & 0.179 & 0.081 & \textbf{0.358} \\
 & MCMC & 82.87 & \textbf{0.178} & 0.081 & 0.365 \\
 \bottomrule
\end{tabular}}
...
Table \ref{tab:ex4static} includes active-signal RMSE and null-coefficient MAE summaries stored in the \code{exalStaticDiagnostic} objects returned by \code{diagnostics()}. The results reinforce the visual comparison in Figure \ref{fig:ex4static}. The MCMC posterior-simulation fit has the smaller holdout quantile RMSE at \(p_0 = 0.05\) and \(p_0 = 0.25\), while LDVB is smaller at \(p_0 = 0.50\). MCMC gives slightly smaller active-signal RMSE in all three fits, whereas null-coefficient MAE favors LDVB for \(p_0 = 0.05\) and \(p_0 = 0.25\) and is essentially tied at the median. The LDVB fit is nevertheless substantially faster in all three cases, with runtime reductions of roughly 4-fold at \(p_0 = 0.05\), 7-fold at \(p_0 = 0.25\), and 14-fold at \(p_0 = 0.50\). In this benchmark, the LDVB approximation is useful as a rapid first-pass screen, while static MCMC provides a more expensive posterior-simulation comparison when interval accuracy or posterior tail behavior is central.
