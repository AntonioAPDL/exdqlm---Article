...
These appendices collect the implementation details most relevant for checking
the \pkg{exdqlm} software contribution. They are intentionally selective. The
main text gives the user-facing workflow, examples, and package interface; the
package help pages and source code document function-level arguments and
backend options; the replication materials regenerate the reported figures,
tables, and benchmark quantities. The appendices below record the posterior
targets, nonconjugate Laplace--delta variational Bayes (LDVB) blocks,
shrinkage-prior updates, diagnostic quantities, and posterior-predictive
synthesis algorithm used by the package.

Table~\ref{tab:app_roadmap} summarizes the appendix organization. Standard
Gaussian filtering, smoothing, and backward-sampling recursions are cited and
used in the package, but not rederived here.
...
\midrule
A. Scope and notation &
Defines the distributional conventions used by the package. \\
B. Posterior targets and model blocks &
Records compact dynamic, static, and transfer-function targets. \\
C. Laplace--delta VB implementation &
Shows the exAL scale--skewness block and its model-specific moments. \\
D. Static \code{rhs\_ns} shrinkage block &
Records the Nishimura--Suchard regularized horseshoe updates used in static
regression. \\
E. Forecasting, diagnostics, and synthesis &
Defines the predictive scoring quantities and post hoc synthesis algorithm. \\
F. Numerical identities and backend notes &
Lists the small set of moment and entropy identities used by the ELBO
calculations and points to package-level backend documentation. \\
\bottomrule
\end{tabularx}
\endgroup
\caption{Compact roadmap for the appendices.}
...
The factor-specific entropy and moment identities used in this decomposition
are listed in Section~\ref{sec:numerical_blocks}. For AL/DQLM fits,
...
\section{Selected numerical identities and backend notes}
\label{sec:numerical_blocks}

\subsection{Moment and entropy identities}
\label{sec:elbo_building_blocks}

The ELBO calculations use a small set of reusable entropy identities. If
$\vect{x}\sim\N(\vect{m},\mat{V})$ in dimension $d$, then
\begin{equation}
H\{\N(\vect{m},\mat{V})\}
=\frac{1}{2}\log\{(2\pi e)^d|\mat{V}|\}.
\label{eq:entropy_gaussian}
\end{equation}
For $x\sim\IG(a,b)$ under the convention above,
\begin{equation}
H\{\IG(a,b)\}
=
...
\label{eq:entropy_ig}
\end{equation}
For $x\sim\GIG(\lambda,\chi,\psi_{\mathrm{GIG}})$,
\begin{align}
H(x)
&=
\log\left\{
2K_\lambda(\sqrt{\chi\psi_{\mathrm{GIG}}})
\left(\frac{\chi}{\psi_{\mathrm{GIG}}}\right)^{\lambda/2}
\right\}\nonumber\\
&\quad
-(\lambda-1)\E(\log x)
+\frac{1}{2}\chi\E(x^{-1})
+\frac{1}{2}\psi_{\mathrm{GIG}}\E(x),
\label{eq:entropy_gig}
\end{align}
with moments from \eqref{eq:gig_moments}. For a positive truncated normal
$x\sim\TNp(\mu,V)$, the entropy is computed from the corresponding normal
entropy plus the log normalizing probability and the standard correction for
left truncation:
\begin{equation}
H\{\TNp(\mu,V)\}
=
\frac{1}{2}\log(2\pi e V)
+\log\Phi(\mu/\sqrt V)
-\frac{\mu}{2\sqrt V}
\frac{\phi(\mu/\sqrt V)}{\Phi(\mu/\sqrt V)}.
...
\subsection{Slice sampler and backend options}
\label{sec:slice}
...
These are documented in the package help pages and README
because they are runtime controls rather than additional model assumptions.
