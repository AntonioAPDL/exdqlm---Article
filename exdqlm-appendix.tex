...
\section{Scope and relation to the main text}
\label{sec:app_scope}

The main text describes the user-facing workflow for \pkg{exdqlm}: extended
dynamic quantile linear model (exDQLM) and dynamic quantile linear model
(DQLM) fitting, transfer-function models, static extended asymmetric
Laplace (exAL) and asymmetric Laplace (AL) regression, forecasting,
diagnostics, and posterior-predictive synthesis. This appendix gives the
corresponding posterior targets, full conditional distributions, Markov chain
Monte Carlo (MCMC) and variational Bayes (VB) updates, Laplace--delta
variational Bayes (LDVB) approximations, evidence lower bound (ELBO)
components, and numerical routines used by the package. Readers can use this
appendix to verify the posterior targets, update equations, and diagnostic
quantities behind the exported functions. The derivations build on the AL
...
\citep{yan2025family,me}, Gaussian state-space filtering,
Rauch--Tung--Striebel (RTS) smoothing, and forward-filtering
backward-sampling (FFBS) recursions described in
\aA{Section~\ref{sec:gaussian_state_update}, the nonconjugate variational
approximations of \citet{wang2013nonconjugatevb}, and the \code{rhs\_ns}
prior \citep{nishimura2023shrunken}. The Laplace--delta sections below give
the exAL/exDQLM specialization of that generic nonconjugate-VI strategy used
by the package.}

Table~\ref{tab:app_roadmap} gives the main navigation map, linking each
article topic to the corresponding appendix sections and exported package
...
The notation follows the main text. In particular, $\mat{F}_t$ denotes the
...
\toprule
Article topic or workflow & Main text location & Appendix location &
...
\end{tabularx}
\endgroup
\caption{Roadmap connecting the main text, appendix derivations, and
exported \pkg{exdqlm} routines or computational blocks.}
\label{tab:app_roadmap}
\end{table}
...
\end{tabularx}
\endgroup
\caption{Core notation used throughout the appendices. The shorthand
$p_\gamma$, $A_\gamma$, $B_\gamma$, and $C_\gamma$ refers to functions of
$(p_0,\gamma)$ defined in the main text.}
...
has density $\sigma^{-1}\exp(-x/\sigma)$, matching the main text's
...
rules described in the main text; once the prediction covariance is formed,
...
\begin{appalgorithm}{Dynamic DQLM MCMC}
...
\begin{appalgorithm}{Dynamic DQLM VB}
...
\begin{appalgorithm}{Dynamic exDQLM MCMC}
...
\begin{appalgorithm}{Dynamic exDQLM LDVB}
...
\begin{appalgorithm}{Transfer-function state augmentation and fitting}
...
described in Section~\ref{sec:static_rhs}. Thus this appendix covers the
...
\begin{appalgorithm}{Static AL/exAL MCMC}
...
\begin{appalgorithm}{Static AL/exAL LDVB}
...
\code{rhs\_ns} is the conditionally conjugate regularized horseshoe variant
used for sparse static regression. These details are needed when fitting static
models with \code{beta_prior = "rhs\_ns"}. The prior enters the static Gaussian update for
...
recursion in the main text gives
\begin{equation}
...
bands for residuals. The reported Kullback--Leibler (KL) summaries are deterministic
one-dimensional normality diagnostics for the MAP standardized forecast-error
sample. The forward value estimates \(\KL(P_e\,||\,N(0,1))\), where \(P_e\) is the
diagnostic-error distribution represented by \(\{\widehat e_t\}\); the flipped
value estimates the reversed diagnostic \(\KL(N(0,1)\,||\,P_e)\). The package
default uses a deterministic standard-normal quantile grid rather than a random
reference sample, so repeated diagnostic calls are reproducible for fixed fitted
objects.
...
\code{exdqlmDiagnostics()} uses an equally spaced grid from 0.01 to 0.99 with
equal weights \(w_k=1/K\). User-supplied weights are normalized to sum to one.
For target quantile $p_0$, the posterior predictive loss criterion (PPLC)
returned as \code{pplc} is
\begin{equation}
\widehat{\PPLC}
=
\sum_t
...
Section~\ref{sec:app_distributional_conventions}, then
\begin{equation}
...
Table~\ref{tab:app_backendopts} lists the package-level options used for
backend routing and ELBO monitoring. These are global \code{options(...)}
settings; they are separate from the model-specific inference controls passed
directly to the fitting functions.
...
\bottomrule
\end{tabularx}
\caption{Package-level runtime options exposed through \code{options(...)} for backend routing and ELBO monitoring in \pkg{exdqlm}. Function-level inference controls are documented separately in the function interfaces.}
\label{tab:app_backendopts}
\end{table}
