...
\citep{yan2025family,me}, Gaussian state-space filtering, smoothing, and FFBS
recursions described in Section~\ref{sec:gaussian_state_update},
...
deterministically by the Rauch--Tung--Striebel backward smoother applied to the
...
$\mat{R}_t$, is formed during the prediction step \citep{west1985dynamic}. For
example, scalar discounting can be written as
\[
\mat{R}_t
=
\delta^{-1}\mat{G}_t\mat{C}_{t-1|t-1}\mat{G}_t^\top,
\]
...
\]
Componentwise discounting applies the same construction blockwise. Once the
prediction covariance $\mat{R}_t$ is formed, the filtering, backward-sampling,
and smoothing equations above are unchanged.

Table~\ref{tab:gaussian_state_update_map} gives the offset and variance used
by each dynamic algorithm. The variational rows are obtained by completing the
square in the expected Gaussian log likelihood.
\begin{table}[!htb]
\centering
\begingroup
\TableStyle
