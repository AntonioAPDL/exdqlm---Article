...
stochastic helper paths so they use serial \proglang{R}-controlled
random-number streams, adds tests for repeated seed behavior, and sets more
stable default exAL scale--skewness updates for MCMC and LDVB.

\section*{Summary of the Revision}

\begin{itemize}
\item We replaced the previous replication interface with a flat
  reviewer-facing \code{code.R}. The script does not call \code{source()}, read
  or write saved fit objects, inspect Git metadata, or require local paths.
\item The primary command is now the JSS-style batch command
  \code{R CMD BATCH --vanilla code.R code.Rout}, with native thread variables
  set before \proglang{R} starts. The full run produces \code{code.Rout} and
  \code{Rplots.pdf} at the archive root.
\item We added \code{code-fast.R}, a reduced code-checking script. It follows
  the same structure with smaller computation settings and is explicitly marked
  as non-authoritative for manuscript numerical values.
...
\item The manuscript now includes a computational-details paragraph describing
  ...
  backend/thread configuration, and the role of the full and reduced scripts.
\item The README and reproducibility notes now describe only the public
  replication commands and expected outputs. Development-only scripts remain in
  the repository but are not presented as the public JSS execution path.
\end{itemize}
...
The revised archive centers on one authoritative script, \code{code.R}. It is a
flat, commented \proglang{R} file ordered by the manuscript examples. Running
\code{R CMD BATCH --vanilla code.R code.Rout} now performs the full manuscript
...
archive design. The optional \code{code-fast.R} file is included only as a
reduced code-checking script for readers who want to verify the workflow without
running the full computation. It is labeled as non-authoritative for manuscript
values.

\commentheading{2. Exact reproducibility and computing environment}

The public scripts now set
\[
...
\]
They also stop immediately unless the loaded package is \pkg{exdqlm} version
1.1.1.

While investigating the differences reported by the editors, we found that some
compiled stochastic helper routines in version 1.1.0 could use OpenMP worker
threads or private C++ seeds in ways that were not appropriate for exact
seed-based replication. Version 1.1.1 corrects these paths by using serial
\proglang{R}-controlled random-number streams. The same patch also uses an
exact scale-collapsed gamma update for exAL MCMC and a structured
\(q(\gamma)q(\sigma\mid\gamma)\) scale--skewness factor for LDVB. We added
package tests for repeated-seed behavior in the compiled samplers, dynamic
MCMC, static MCMC, and the structured scale--skewness factor, and the package
passes \code{R CMD check --as-cran} under \proglang{R} 4.6.0.
...
functions used to write the manuscript PNG files. The package patch in version
1.1.1 removes the compiled stochastic RNG/thread behavior that could make
MCMC-derived outputs differ under repeated seeds or different thread settings.
The response does not claim this was the only possible source of the previously
observed visual differences; instead, the revised archive makes the figure
provenance direct and rerunnable from the submitted script.
...
\code{analysis/manuscript/outputs/figures/}.
...
\code{analysis/manuscript/outputs/tables/}.
...
\item targeted package repeatability tests for compiled samplers, dynamic MCMC,
  and static MCMC;
\item the package test suite;
\item \code{R CMD build} and \code{R CMD check --no-manual --run-donttest} for
  \code{exdqlm\_1.1.1.tar.gz};
\item \code{R CMD BATCH --vanilla code-fast.R code-fast.Rout};
\item \code{R CMD BATCH --vanilla code.R code.Rout};
\item manuscript and response compilation; and
\item archive extraction and execution checks from a directory without Git
  metadata.
\end{itemize}
...
batch artifacts requested by the editors.
