...
\item \code{exdqlm\_1.1.2.tar.gz}: software source.
\item \code{exdqlm-jss-replication.tar.gz}: replication archive.
\item \code{response-to-editor.pdf}: this response.
\end{itemize}

The package source is version 1.1.2 of \pkg{exdqlm}. This is a narrow
...
While investigating the differences reported by the editors, we identified a
fixed-seed platform discrepancy in the fast dynamic MCMC path and corrected it
before rerunning the article. Version 1.1.2 is the public CRAN package version
used for the resubmission, and the full JSS replication was rerun with this
version.
...
\begin{itemize}
\item targeted package repeatability checks for the fixed-seed dynamic MCMC
  path relevant to the manuscript;
\item the package test suite and CRAN submission checks for
  \code{exdqlm\_1.1.2.tar.gz};
